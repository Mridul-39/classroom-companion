...
export function detectMessageIntent(message, role) {
  ...
  if (role === 'teacher') {
    ...
    const looksLikeRequestFile =
      /\b(ask|tell|get|have|prompt)\b.{0,60}\b(share|send|attach|submit|upload|provide)\b.{0,50}\b(file|document|doc|word|pdf|image|photo|attachment|work)\b/i.test(lower) ||
      /\b(re[- ]?open|resubmit|re[- ]?submit)\b.{0,40}\b(assignment|submission)\b/i.test(lower);
    if (looksLikeRequestFile) return 'REQUEST_FILE';
  ...
  if (role === 'student') {
    ...
    // "completed/finished the assignment" → submission, only when there's no partial indicator
    const hasPartialIndicator = /\b(\d+\s*%|percent|half|partial|almost|nearly|halfway|some of|part of)\b/i.test(lower);
    if (
      !hasPartialIndicator &&
      (
        /\b(completed|finished)\s+(the\s+|my\s+|this\s+)?(assignment|essay|homework|task|work)\b/i.test(lower) ||
        /\bdone\s+with\s+(the\s+|my\s+|this\s+)?(assignment|essay|homework|task|work)\b/i.test(lower)
      )
    ) {
      return 'SUBMIT';
    }
...
function parseDueDateFromText(rawText) {
  ...
  // Specific words first, so "by tomorrow" doesn't get swallowed by a stray number elsewhere.
  if (/\b(by\s+)?tonight\b/.test(lower)) {
    const d = new Date(now);
    d.setHours(22, 0, 0, 0);
    if (d.getTime() <= now.getTime()) d.setDate(d.getDate() + 1);
    return d;
  }
  if (/\b(by\s+|due\s+)?tomorrow\b/.test(lower)) {
  ...
  // Generic "N <unit>" — matches "due in 3 days", "deadline is 2 days", "in 5 hours", etc.
  // Avoid the "1000 words" trap by only matching time units.
  const generic = lower.match(/(\d+)\s*(minutes?|mins?|hours?|hrs?|days?|weeks?)\b/);
  if (generic) {
    const n = Number(generic[1]);
    const unit = generic[2];
    if (/^min/.test(unit)) return new Date(now.getTime() + n * 60 * 1000);
    if (/^h/.test(unit)) return new Date(now.getTime() + n * 60 * 60 * 1000);
    if (/^day/.test(unit)) return new Date(now.getTime() + n * 24 * 60 * 60 * 1000);
    if (/^week/.test(unit)) return new Date(now.getTime() + n * 7 * 24 * 60 * 60 * 1000);
  }
...
const NAME_STOPWORDS = new Set([
  'a', 'an', 'the', 'new', 'assignment', 'homework', 'task', 'essay', 'project',
  'work', 'to', 'for', 'with', 'about', 'on', 'in', 'and', 'or', 'where', 'who',
  'that', 'which', 'when', 'why', 'how', 'he', 'she', 'they', 'it', 'his', 'her',
  'them', 'their', 'will', 'shall', 'should', 'must', 'needs', 'need', 'wants',
  'want', 'has', 'have', 'is', 'are', 'was', 'were', 'be', 'do', 'does', 'can',
  'could', 'would', 'words', 'word',
]);

function extractStudentNameFromAssignText(rawText, rosterNames = []) {
  // Prefer matching a real roster name (longest match wins, case-insensitive).
  if (Array.isArray(rosterNames) && rosterNames.length > 0) {
    const lower = rawText.toLowerCase();
    const sorted = [...rosterNames]
  ...
  // Regex fallbacks. Cap captured names at the first stopword so we don't
  // grab "mridul where he" when the text is "to mridul where he needs to ...".
  const candidates = [
    rawText.match(/"[^"]+"\s*,?\s*to\s+([a-z][a-z']*(?:\s+[a-z][a-z']*){0,3})/i),
    rawText.match(/\bto\s+([a-z][a-z']*(?:\s+[a-z][a-z']*){0,2})\s*(?:,|\.|the\s+deadline|deadline|due|$)/i),
    rawText.match(/\bassign\b[^]*?\bto\s+([a-z][a-z']*(?:\s+[a-z][a-z']*){0,2})/i),
    rawText.match(/\bassign\s+([a-z][a-z']*(?:\s+[a-z][a-z']*){0,1})\b/i),
  ];
...
function fallbackAssignIntent(rawText, rosterNames = []) {
  if (!rawText || typeof rawText !== 'string') return null;

  const text = rawText.trim().replace(/\s+/g, ' ');
  const dueDate = parseDueDateFromText(text);
  const studentName = extractStudentNameFromAssignText(text, rosterNames);

  // Return partial info so the caller can ask for what's missing.
  if (!dueDate && !studentName) return null;
  ...
  if (quoted) {
    ...
    const stripped = text
      ...
      .replace(/\b(in|within)\s+\d+\s*(minutes?|mins?|hours?|hrs?|days?|weeks?)\b/gi, '')
      .replace(/\b\d+\s*(minutes?|mins?|hours?|hrs?|days?|weeks?)\b/gi, '')
      .trim();
...
export async function parseAssignIntent(teacherMessage, allowedStudentNames = []) {
  ...
  const rosterRule =
    allowedStudentNames.length > 0
      ? `The teacher may ONLY assign to these students: ${allowedStudentNames.join(', ')}. Use the best matching name (first name or full name). If no roster match, set studentName to null.`
      : 'This teacher has no students yet. Set studentName to null.';

  try {
    const prompt = `You extract assignment creation intent from a teacher message. Respond with ONLY valid JSON.

Now is ${nowIso}. Compute dueDate relative to now.

Fields (use null when missing — do not invent):
- studentNames: ARRAY of student names. One name per intended recipient. Use ["__ALL__"] if the teacher means everyone/the whole class/all students. Each name must be on the roster.
...
Output: {"studentNames":["riya"],"title":"Essay on Photosynthesis","description":"500 words","dueDate":"<3 days from now ISO>"}

Input: assign to both ronak and mridul a graph traversal coding task, due in 2 days
Output: {"studentNames":["ronak","mridul"],"title":"Graph Traversal Coding Task","description":"Write a graph traversal program","dueDate":"<2 days from now ISO>"}

Input: give everyone a math worksheet due tomorrow
Output: {"studentNames":["__ALL__"],"title":"Math Worksheet","description":"Complete the math worksheet","dueDate":"<tomorrow ISO>"}

Input: can you assign "create a telegram bot" to Mridul Goyal, deadline 6 hours from now
Output: {"studentNames":["Mridul Goyal"],"title":"Create a telegram bot","description":"create a telegram bot and share the github repo link","dueDate":"<6h from now ISO>"}

Input: assign mridul an essay of 1000 words
Output: {"studentNames":["mridul"],"title":"Essay of 1000 words","description":"Write an essay of 1000 words","dueDate":null}

Input: ${normalized}
Output:`;

    const output = await callLLM(prompt, { temperature: 0.0, maxOutputTokens: 320 });
    const parsed = extractJson(output);
    if (parsed) {
      let studentNames = [];
      if (Array.isArray(parsed.studentNames)) {
        studentNames = parsed.studentNames
          .map((n) => (n == null ? null : String(n).trim()))
          .filter(Boolean);
      } else if (parsed.studentName) {
        studentNames = [String(parsed.studentName).trim()];
      }
      ...
      if (parsed.dueDate) {
        ...
        if (!Number.isNaN(d.getTime()) && d.getTime() > Date.now()) dueDate = d;
      } else if (Number.isFinite(parsed.dueDays)) {
        dueDate = new Date(Date.now() + Number(parsed.dueDays) * 24 * 60 * 60 * 1000);
      }
      if (studentNames.length > 0 || dueDate) {
        return {
          studentNames,
          studentName: studentNames[0] ?? null,
          title,
          description,
          dueDate,
        };
  ...
  const fallback = fallbackAssignIntent(normalized, allowedStudentNames);
  if (fallback) {
    return {
      studentNames: fallback.studentName ? [fallback.studentName] : [],
      studentName: fallback.studentName,
      title: fallback.title,
      description: fallback.description,
      dueDate: fallback.dueDate ? new Date(fallback.dueDate) : null,
    };
  }
  return null;
}
